...
public class WindowManager: NSObject, NSWindowDelegate {
    ...
    public func setFrame(_ args: [String: Any]) {
        let frame = NSRect(
            x: CGFloat(truncating: args["x"] as! NSNumber),
            y: CGFloat(truncating: args["y"] as! NSNumber),
            width: CGFloat(truncating: args["width"] as! NSNumber),
            height: CGFloat(truncating: args["height"] as! NSNumber)
        )

        mainWindow.setFrame(frame, display: false, animate: true)
    }
    ...
    public func getIconImage(_ args: [String: Any]) -> FlutterStandardTypedData?{
