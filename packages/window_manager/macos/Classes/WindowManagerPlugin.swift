...
public class WindowManagerPlugin: NSObject, FlutterPlugin {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "getCursorPos":
            ...
            var x = Int32(cursorPos.x)
            var y = Int32(cursorPos.y)
            let data = Data(bytes: &x, count: MemoryLayout<Int32>.size) + Data(bytes: &y, count: MemoryLayout<Int32>.size)
            result(FlutterStandardTypedData(int32: data))
            break
        case "getLeftClick":
        ...
        case "getPasteboard":
            ...
            result(pasteboard.pasteboardItems?.count ?? -1)
            break
        case "clearPasteboard":
            let pasteboard = NSPasteboard(name: .drag)
            pasteboard.clearContents()
            result(true)
            break
        case "getIconImage":
            let iconImage = windowManager.getIconImage(args)
            result(iconImage)
            break
        default:
