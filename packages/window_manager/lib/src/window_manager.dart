...
class WindowManager {
  ...
  Future<Offset> getCursorPos() async {
    final Int32List cursorPos = await _channel.invokeMethod('getCursorPos');
    return Offset(cursorPos.first.toDouble(), cursorPos.last.toDouble());
  }

  Future<bool> getLeftClick() async {
    return await _channel.invokeMethod('getLeftClick');
  }

  Future<int> getPasteboard() async {
    return await _channel.invokeMethod('getPasteboard');
  }

  Future<void> clearPasteboard() async {
    return await _channel.invokeMethod('clearPasteboard');
  }
  ...
  Future<Uint8List?> getIconImage(String path) async {
    final Uint8List? image = await _channel.invokeMethod<Uint8List>(
      'getIconImage',
      {
        'path': path,
      },
